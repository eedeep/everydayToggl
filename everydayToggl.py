...
def parse_time_entries_in_file(file_path):
	...
	with open(file_path) as timesheet_log_file:
		...
		for line in lines:
			matches = re.match('s:(?P<start>.*? )e:(?P<end>.*? )(?P<description>.*)$', line)
			if matches:
				try:	
				...
				logger.error("Could not parse 'start', 'end' and 'description' out of the following line: {0} from file at {1}".format(line, file_path))

	return entries

	
def get_time_entries_for_yesterday():
...
get_time_entries_for_yesterday()
